#!/usr/bin/env python3
"""
Teste Melhorado de Descoberta de Movimentos G1.
Versão que detecta melhor movimentos rápidos.
"""
...
class G1ImprovedDiscovery:
    """Descoberta melhorada de movimentos do G1."""
    ...
    def test_arm_id_improved(self, test_id):
        """Testa um ID de braço com detecção melhorada."""
        print(f"\n🔄 Testando Arm ID: {test_id}")
        
        # Verificar se já foi testado
        if str(test_id) in self.results and self.results[str(test_id)]["status"] == "working":
            print(f"   ⏭️  ID {test_id} já testado e funcional: {self.results[str(test_id)]['name']}")
            return True
        ...
        try:
            ...
            if result == 0:
                ...
                # Aguardar mais tempo para detectar movimentos rápidos
                print(f"   ⏳ Aguardando execução (movimentos rápidos podem ser difíceis de detectar)...")
    ...
    def test_fsm_id_improved(self, test_id):
        """Testa um ID de FSM com detecção melhorada."""
    ...
    def test_range_improved(self, start_id, end_id, test_type="arm"):
        """Testa um range de IDs com melhorias."""
        ...
        for test_id in range(start_id, end_id + 1):
            ...
            if test_type == "arm":
                self.test_arm_id_improved(test_id)
            else:
                self.test_fsm_id_improved(test_id)
    ...
    def show_working_movements(self):
        ...
        working = [f"{id}: {data['name']}" for id, data in self.results.items() if data["status"] == "working"]
        if working:
            print(f"\n🎯 MOVIMENTOS FUNCIONAIS DESCOBERTOS:")
            print("=" * 50)
            for movement in working:
                print(f"   {movement}")
        else:
    ...
    def run_discovery(self):
        ...
        print("🔍 DESCOBERTA MELHORADA DE MOVIMENTOS G1")
        print("=" * 60)
        ...
        try:
            ...
            self.test_range_improved(start_id, end_id, test_type)
...
def main():
    """Função principal."""
    print("🔍 DESCOBERTA MELHORADA G1")
    print("=" * 60)
    
    discovery = G1ImprovedDiscovery()
    ...
    print("\n✅ SDK inicializado!")
    print("🎯 OBJETIVO: Descobrir movimentos funcionais (versão melhorada)")
    print("⚠️  MELHORIAS: Detecta melhor movimentos rápidos")
